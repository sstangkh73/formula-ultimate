"""Import Work 083 exact STEP artifacts, retain separate solids, and save FCStd."""
...
from formula_ultimate.subsystems.ground_interaction import canonical_sha256  # noqa: E402
...
def main() -> int:
    manifest_path = Path(os.environ["FORMULA_ULTIMATE_W083_MANIFEST"])
    step_root = Path(os.environ["FORMULA_ULTIMATE_W083_STEP_ROOT"])
    output_report = Path(os.environ["FORMULA_ULTIMATE_W083_FREECAD_REPORT"])
    output_fcstd = Path(os.environ["FORMULA_ULTIMATE_W083_FCSTD"])
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))

    document = App.newDocument("GroundInteractionCandidate001")
    records = []
    ...
    if len(assembly_shape.Solids) != len(records) or not assembly_shape.isValid():
        raise RuntimeError("assembly did not preserve five valid separate solids")
